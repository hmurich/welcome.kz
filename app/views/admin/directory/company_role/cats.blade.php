...
@section('content')
{{ Form::open(array('url'=>action('AdminRoleController@postCat', $filter->id), 'method' => 'post', 'role'=>'form')) }}
    ...
    <tbody>
        @foreach ($items as $i)
            <tr>
                ...
                <td><a href='{{ action('AdminRoleController@getDeleteCat', array($i->id)) }}'>Удалить</a></td>
            </tr>
        @endforeach
    </tbody>
	<tfoot>

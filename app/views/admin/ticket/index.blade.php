    ...
    <tbody>
        @foreach ($items as $i)
            <tr>
                ...
                <td>{{ $i->updated_at }}</td>
                <td>
                    <a href='{{ action('AdminTicketController@getHistory', array($i->status_id, $i->id)) }}'>История</a>
                </td>
                <td>
                    <a href='{{ action('AdminTicketController@getItem', array($i->status_id, $i->id)) }}'>Ответить</a>
                </td>
            </tr>
        @endforeach
    </tbody>
